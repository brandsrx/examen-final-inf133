...
def create_reservation():
    ...
    new_reservation_date = datetime.fromisoformat(reservation_date) if reservation_date is not None else None
    if not user_id or not restaruant_id or not reservation_date or not num_guests or status is None:
        return jsonify({"error":"Faltan datos requeridos"}),400
    reservas = Reservas(user_id,restaruant_id,new_reservation_date,num_guests,special_requests,status)
...
def update_reservations(id):
    ...
    new_reservation_date = datetime.fromisoformat(reservation_date) if reservation_date is not None else None
    try: 
        reserva.update(user_id,restaruant_id,new_reservation_date,num_guests,special_requests,status)
