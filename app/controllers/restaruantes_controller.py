...
@rest_bp.route("/restaruants", methods=["GET"])
@jwt_required
...
@rest_bp.route("/restaruants/<int:id>",methods=["GET"])
@jwt_required
@roles_required(roles=["admin","customer"])
def get_restaruante(id):
    ...
    return jsonify({"error":"Restaruante no encontrado"}),404
@rest_bp.route("/restaruants",methods=["POST"])
@jwt_required
...
def create_restaruante():
    ...
    restaruante = Restaruantes(name=name,addres=address,city=city,phone=phone,description=description,rating=rating)
    restaruante.save()
    
    return jsonify(render_restaruant_detail(restaruante)),201

@rest_bp.route("/restaruants/<int:id>",methods=["PUT"])
@jwt_required
@roles_required(roles=["admin"])
def update_restaruante(id):
    ...
    try: 
        restaruante.update(name=name,description=description,price=price,stock=stock)
        return jsonify(render_restaruant_detail(restaruante)),200
    except Exception as e:
        return jsonify({"error": str(e)}), 401
    

@rest_bp.route("/restaruants/<int:id>",methods=["DELETE"])
@jwt_required
@roles_required(roles=["admin"])
